 ...
 * @throws moodle_exception
 */
function get_question_request(&$client, $path, $headers, $post_data)
{
    try {
        return $client->post(
            $path,
        ...
        exit();
        // Networking error, Throw a Moodle Exception.
        throw new moodle_exception('getquestionsnotavailable', 'dmelearn');
    }
}
...
function elmo_parse_config_page_scripts($module, $page, $data)
{
    if (($module === $data['module']) && ($page === $data['page'])) {
        $src = ELMO_WEB_COURSE_JAVASCRIPT . $data['script'];
...
function support_course_num($version_num)
{
    ...
    $supported = [1, 2, 2.1, 3, 4];

    return in_array($version_num, $supported, false);
}
