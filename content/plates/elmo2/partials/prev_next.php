...
<div class = "prev_next row-fluid">
    ...
    <a class="button-flat-primary next_button span6"
       href="<?=$lms_content_url?>&module=<?=$next_url['module']?>&page=<?=$next_url['page']?>">NEXT
...
<script>
    ...
    {
        <?php if ($next_url) : ?>
        window.location = '<?=$lms_content_url?>&module=<?=$next_url['module']?>&page=<?=$next_url['page']?>';
        <?php endif; ?>
    }
    function goToPrevPage()
    {
        <?php if ($previous_url) : ?>
        window.location = '<?=$lms_content_url?>&module=<?=$previous_url['module']?>&page=<?=$previous_url['page']?>';
        <?php endif; ?>
    }
</script>
<?php endif; ?>
